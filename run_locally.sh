...
sed -i "s+interlog.cluster.ai.wu.ac.at+`ip -4 -o address | grep eth0 | awk '{print $4}' | awk -F"/" '{print $1}'`:8000+g" ./webapp/static/index.js
sed -i "s+interlog.cluster.ai.wu.ac.at+`ip -4 -o address | grep eth0 | awk '{print $4}' | awk -F"/" '{print $1}'`:8000+g" ./webapp/static/FilterContainer.js
